...
export class MediaFileEraser {
  public async erase(entity: MediaEntity, mediaAlias: MediaAlias, entityAlias: EntityAlias): Promise<void> {
    ...
    const filePath = this.setFilePath(mediaAlias, entityAlias, fileName);

    await fs.unlink(filePath);
  }

  public setFilePath(mediaAlias: MediaAlias, entityAlias: EntityAlias, fileName?: string): string {
    return path.join(process.cwd(), "uploads", mediaAlias, entityAlias, fileName ?? "");
  }
}
